...
function mockContacts() {
  return [
    { id: '1', name: '成员甲', email: 'member-a@example.com' },
    { id: '2', name: '成员乙', email: 'member-b@example.com' },
    { id: '3', name: '成员丙', email: 'member-c@example.com' },
    { id: '4', name: '成员丁', email: 'member-d@example.com' },
    { id: '5', name: '成员戊', email: 'member-e@example.com' },
  ]
}
...
function mockMeetingDetail(id) {
  return {
    ...
    transcript: [
      ...
      { id: 2, start_ms: 12000, end_ms: 28000, text: '成员甲：根据上个季度的数据，我们的DAU已经达到了50万，但是用户留存率在第7天出现了明显下滑，大概是在35%左右。' },
      { id: 3, start_ms: 28000, end_ms: 45000, text: '成员乙：我这里的分析表明，主要原因是新用户引导流程太复杂。我建议我们简化注册后的前三步操作，把步骤从7步减少到3步。' },
      { id: 4, start_ms: 45000, end_ms: 62000, text: '成员丙：同意，另外用户反馈系统通知太多，导致通知疲劳。我来负责梳理通知策略，争取本月底完成方案。' },
      { id: 5, start_ms: 62000, end_ms: 85000, text: '成员丁：技术层面，我们需要对搜索模块进行重构，现在的响应时间平均是2.3秒，目标要优化到800ms以内。这个工作量大概需要两周。' },
    ...
    action_items: [
      { id: 1, owner_id: '3', owner_name: '成员丙', content: '梳理并重新设计App内通知推送策略，减少日均通知数量至3条以内', due_date: '2026-03-31', status: 'pending' },
      { id: 2, owner_id: '4', owner_name: '成员丁', content: '完成搜索模块架构重构，将P50响应时间优化至800ms以内', due_date: '2026-03-20', status: 'pending' },
      { id: 3, owner_id: '2', owner_name: '成员乙', content: '重新设计新用户注册引导流程，步骤从7步简化为3步，并完成A/B测试方案', due_date: '2026-03-15', status: 'pending' },
      { id: 4, owner_id: '1', owner_name: '成员甲', content: '输出第7天留存率优化专项报告，含数据基线与改进目标', due_date: '2026-03-10', status: 'pending' },
    ],
  }
}
