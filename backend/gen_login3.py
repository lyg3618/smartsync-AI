import os, pathlib
# Chinese labels as unicode escapes - no encoding issues
title      = "\u667a\u80fd\u4f1a\u8bae\u6d1e\u5bdf\u4e0e\u4efb\u52a1\u4e2d\u67a2"
usr_ph     = "\u7528\u6237\u540d"
pwd_ph     = "\u5bc6\u7801"
btn_login  = "\u767b\u5f55\u7cfb\u7edf"
  ...
  <div class="login-page">
    ...
    <div class="login-card fade-slide-up">
      <div class="brand">
        ...
        <div><h1 class="brand-name">SmartSync</h1><p class="brand-sub">{title}</p></div>
      </div>
      <el-form :model="form" size="large" @submit.prevent="handleLogin">
        <el-form-item><el-input v-model="form.username" placeholder="{usr_ph}" prefix-icon="User" /></el-form-item>
        <el-form-item><el-input v-model="form.password" type="password" placeholder="{pwd_ph}" prefix-icon="Lock" show-password /></el-form-item>
        <div v-if="error" class="error-tip">{{{{ error }}}}</div>
        <el-button type="primary" native-type="submit" class="login-btn" :loading="loading" size="large" round>{btn_login}</el-button>
      </el-form>
      <div class="login-accounts">
        <p class="accounts-label">{quick_lbl}</p>
...
const quickAccounts = [
  {{ label: '{u_admin}', username: 'admin', password: 'admin123' }},
  {{ label: '{u_zhang}', username: 'zhang', password: 'zhang123' }},
  {{ label: '{u_li}',    username: 'li',    password: 'li123' }},
  {{ label: '{u_wang}',  username: 'wang',  password: 'wang123' }},
]
function quickLogin(a) {{ form.value.username = a.username; form.value.password = a.password; handleLogin() }}
async function handleLogin() {{
  if (!form.value.username || !form.value.password) {{ error.value = '{err_empty}'; return }}
  error.value = ''; loading.value = true
  try {{
    const fd = new URLSearchParams()
    fd.append('username', form.value.username); fd.append('password', form.value.password)
    const res = await axios.post('http://localhost:8000/auth/login', fd, {{ headers: {{ 'Content-Type': 'application/x-www-form-urlencoded' }} }})
    const {{ access_token, user }} = res.data
    localStorage.setItem('smartsync_token', access_token)
    localStorage.setItem('smartsync_user', JSON.stringify(user))
    ElMessage.success('{ok_prefix} ' + user.name + '!')
    router.push('/dashboard')
  }} catch (e) {{
    const msg = e.response?.data?.detail || '{err_fail}'
    error.value = msg; ElMessage.error(msg)
...
.account-chip{{padding:5px 14px;border-radius:20px;background:rgba(79,110,246,.1);border:1px solid rgba(79,110,246,.25);color:var(--primary);font-size:12px;cursor:pointer;transition:all .2s;}}
.account-chip:hover{{background:rgba(79,110,246,.2);transform:translateY(-1px);}}
</style>
"""
dst = pathlib.Path(r'd:\Desktop\AI\u4f1a\u8bae\smartsync\frontend\src\views\Login.vue')
dst.write_text(vue, encoding='utf-8')
print(f'Login.vue written: {{dst.stat().st_size}} bytes')