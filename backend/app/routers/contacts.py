...
async def list_contacts(current_user: dict = Depends(get_current_user)):
    ...
    async with pool.acquire() as conn:
        async with conn.cursor() as cur:
            await cur.execute(
                "SELECT id, name, COALESCE(email, ''), COALESCE(collab_no, '') FROM contacts ORDER BY name"
            )
            rows = await cur.fetchall()
    return [{"id": row[0], "name": row[1], "email": row[2], "collab_no": row[3]} for row in rows]
...
async def create_contact(contact: ContactCreate, current_user: dict = Depends(get_current_user)):
    ...
    collab_no = contact.collab_no.strip()
    ...
    async with pool.acquire() as conn:
        async with conn.cursor() as cur:
            try:
                await cur.execute(
                    """
                    INSERT INTO contacts (id, name, email, collab_no, user_id, username, hashed_password, role)
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
                    """,
                    (contact_id, name, email, collab_no, current_user["sub"], username, hashed_password, "user"),
                )
            except Exception:
                raise HTTPException(status_code=400, detail="用户名或邮箱已存在")

    return {"id": contact_id, "name": name, "email": email, "collab_no": collab_no, "username": username}
...
async def update_contact(contact_id: str, contact: ContactUpdate, current_user: dict = Depends(get_current_user)):
    ...
    async with pool.acquire() as conn:
        async with conn.cursor() as cur:
            try:
                await cur.execute(
                    "UPDATE contacts SET name = %s, email = %s, collab_no = %s WHERE id = %s",
                    (name, contact.email.strip(), contact.collab_no.strip(), contact_id),
                )
                if cur.rowcount == 0:
    ...
    return {"id": contact_id, "name": name, "email": contact.email.strip(), "collab_no": contact.collab_no.strip()}
